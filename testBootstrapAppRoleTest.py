...
doAppRoleAssignment = True
if doAppRoleAssignment:
    print ("Adding Graph 2.0 scope to the created SP via PG recommended API")
    ...
    bootstrapClientID = 'b1b71e9b-6abd-45d1-bf61-127e287d6a59'
    bootstrapApp = PublicClientApplication(bootstrapClientID, authority=authority)
    ...
    bootstrapResult = app.acquire_token_by_username_password(username=userName, password=userPassword, scopes=scope)
    targetClientId = appId

    if "access_token" in result:
        # Call a protected API with the access token.
        bootstrapToken = result["access_token"]
        print(bootstrapToken)
        bootstrapEndpoint = graphURI + f"beta/servicePrincipals/targetClientId/appRoleAssignedTo"
        bootstrapHttp_headers = {'Authorization': 'Bearer ' + bootstrapToken,
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'}
        bootstrapData = requests.get(bootstrapEndpoint, headers=bootstrapHttp_headers, stream=False).json()
        print(bootstrapData)
